"use strict"; //использует "строгий режим" тоесть современную спецификацию JS, более строгую к некоторым ошибкам
const $ = el => document.querySelector(el); //задаем переменные
const $$ = ell => document.querySelectorAll(ell); //кстати, эти "$"будут использоваться чтобы не писать всякий раз много кода выбора DOM-элемента
let curSign;
let ifWon = false;
...
//if a developer see this: don't judje me too strictly, some things could be done in better way, but im too lazy to improve 'em !)